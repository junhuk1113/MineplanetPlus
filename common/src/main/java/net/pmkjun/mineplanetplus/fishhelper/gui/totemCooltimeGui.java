...
public class totemCooltimeGui {
    ...
    private static final Identifier TOTEM_ICON = Identifier.fromNamespaceAndPath("pyrofishinghelper","totem.png");
    private static final Identifier TOTEM_SLEEP_ICON = Identifier.fromNamespaceAndPath("pyrofishinghelper","sleepingtotem3.png");
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, Timer timer) {
        ...
        if(this.client.data.toggleTotemtime && this.client.data.isTotemCooldown){
            extract(guiGraphics, TOTEM_SLEEP_ICON, cooldownsecond);
        } else if(this.client.data.toggleTotemtime){
            extract(guiGraphics, TOTEM_ICON, activesecond);
        }
    }

    // render -> extract
    private void extract(GuiGraphicsExtractor guiGraphics, Identifier texture, int second) {
        ...
        if (this.client.data.toggleTotemtimeText) {
            ...
            // drawString -> text
            guiGraphics.text(font, Component.literal(String.format("%02d:%02d", minute, second)), 0, 0, ARGB.white(255));
